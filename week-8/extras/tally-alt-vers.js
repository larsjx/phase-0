...
function tallyVotes() {
  for (var voter in votes) {
    for (var i = 0; i < 4; i++) {
      var role = job[i]
      var person = votes[voter][job[i]]
        if (voteCount[role][person] != null) {
          voteCount[role][person] += 1;
        }
        else {
          voteCount[role][person] = 1;
        }
...
function electionWinner() {
 for (var role in officers) {
  var winner = ""
  var leading_num = 0
  var winning_num = 0
  for (var person in voteCount[role]) {
    var votes = voteCount[role][person]
    if (votes > leading_num) {
      leading_num = votes
      winning_num = leading_num
      winner = person
    }
    officers[role] = winner
  }
}
}
electionWinner()
